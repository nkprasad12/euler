...
public class Problem102 {
  ...
  static boolean containsOrigin(String input) {
    ...
    return hasNegativeXIntercept
        && hasPositiveXIntercept
        && hasNegativeYIntercept
        && hasPositiveYIntercept;
  }

  static Pair<Double, Double> getIntercepts(
    ...
    if (((b <= point2.second() && b >= point1.second())
        || (b >= point2.second() && b <= point1.second()))) {
      yIntercept = b;
    }

    double possibleXIntercept = -1 * b / slope;

    if ((possibleXIntercept >= point2.first() && possibleXIntercept <= point1.first())
        || possibleXIntercept <= point2.first() && possibleXIntercept >= point1.first()) {
